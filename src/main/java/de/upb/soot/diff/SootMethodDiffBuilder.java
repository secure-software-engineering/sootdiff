...
public class SootMethodDiffBuilder extends DiffBuilder {
  ...
  public SootMethodDiffBuilder(SootMethod lhs, SootMethod rhs, ToStringStyle style) {
    ...
    this.append(
            "returnType", lhs.getReturnType().toString(), rhs.getReturnType().toString());

    List<String> lhsParameter =
            lhs.getParameterTypes().stream().map(p -> p.toString()).collect(Collectors.toList());
    List<String> rhsParameter =
            rhs.getParameterTypes().stream().map(p -> p.toString()).collect(Collectors.toList());
